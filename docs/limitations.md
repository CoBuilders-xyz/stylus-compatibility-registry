...
- **Default features:** an entry can record that a crate only builds without std once its default features are off, and the check warns while the dependency keeps them on. Matching is by name, so a version whose defaults are already `no_std`, such as `sha2` and `sha3` 0.11, still warns. Under `--include-transitive` the setting comes from Cargo's resolved feature union instead of the declaration: a crate without a `default` feature never warns, and a dependency elsewhere in the graph can raise the warning for a manifest that is already correct. A dependency inherited from a workspace is read as keeping its defaults on, whatever the workspace declares.
